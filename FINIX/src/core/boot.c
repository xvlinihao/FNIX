...
void* find_multiboot_tag(multiboot_tag_t *tags, uint16_t type)//找到指定类型的multiboot_header tag, 返回指针
{
    multiboot_tag_t *tag;
...
reserved_areas_t read_multiboot_info(multiboot_info_t *mbi)//读取multiboot tag 中的信息
{
    multiboot_tag_t *tag;
