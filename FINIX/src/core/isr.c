...
void isr_init()
{
    ...
    port_byte_out(PIC1, 0x11);//00010001
    port_byte_out(PIC2, 0x11);//00010001

    // set IRQ base numbers for each PIC:ICW2
    port_byte_out(PIC1_DATA, IRQ_BASE);//00100000
    port_byte_out(PIC2_DATA, IRQ_BASE + 8);//00101000

    // use IRQ number 2 to relay IRQs from the slave PIC:ICW3
    port_byte_out(PIC1_DATA, 0x04);//00000100
    port_byte_out(PIC2_DATA, 0x02);//00000010

    // finish initialization:ICW4
    ...
    set_idt_gate(IRQ14, (uint64_t) irq14);
